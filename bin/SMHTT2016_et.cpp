...

{
  ...
  cb.cp().process({"DYL"}).AddSyst(cb, "CMS_mFakeTau_2016 ", "lnN",SystMap<>::init(1.26));    
  ...
  //embedded uncertainties. No embedded available for 2016 yet.
  //********************************************************************************************************************************
...
*/
  ...
  TFile output((((string)std::getenv("CMSSW_BASE"))+"/src/CombineHarvester/Run2HTT_Combine/HTT_Output/Output_"
		+Input.ReturnToken(0)+"/"+"smh2016_et.input.root").c_str(), "RECREATE");
